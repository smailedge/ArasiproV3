...
print ("====機器壹登入成功====")
kl = LINE("uurt714@gmail.com","qwaszx9910425")
    ...
    try:
        ...
        if op.type == 60:
            if op.param2 in settings['blacklist']:
                bot = random.choice([kl,sa,sb,kt])
                bot.kickoutFromGroup(op.param1,[op.param2])              
                bot.cancelGroupInvitation(op.param1,[op.param2])
                bot.preventedJoinByTicket = True 
                cl.updateGroup(bot)
                cl.sendMessage(op.param1, "[警告]\n此人位於黑名單中! ! !")
                ...
                if settings['limit'][sender]['text']['react'] == True:
                    if sender in god:
                        if msg.text in ["cancel"]:
                            if msg.toType == 2:
                                X = sa.getGroup(msg.to)
                                if X.invitee is not None:
                                    gInviMids = (contact.mid for contact in X.invitee)
                                    ginfo = sa.getGroup(msg.to)
                                    sinvitee = str(len(ginfo.invitee))
                                    start = time.time()
                                    for cancelmod in gInviMids:
                                        bot = random.choice([kl,sb,kt,sa])
                    ...
                    if text.lower() == 'help':
                        ...
                        try:
                            ...
                            ret_ += "\n[URLを追加] : https://line.me/ti/p/_SiDniqb4h"
                            cl.sendMessage(to, str(ret_))
                        except Exception as e:
                            cl.sendMessage(msg.to, str(e))
